...
| **01-core.js** | Constants, state, storage (load/save / IndexedDB), date/time utilities, processResets, completion write path (incl. manual closed cycles), stock banner/PFP catalog, unlock + integrity repair helpers |
| **01b-game-presets.js** | **Generated** `GAME_PRESETS` from `presets/*.json` (do not edit by hand; run `node build.js`) |
| **02-modals.js** | Task modal, game modal, Manual Reset / Add completion modals, stock picker, delete game modal, calendar day modal, earnings modal |
| **03-games.js** | addGame (preset Official icon), deleteGame, reallyDeleteGame, toggle functions, currency/earnings helpers |
| **04-shared.js** | Sidebar time, renderTabs, format toggles, game icon / banner helpers |
...
| **10-page-games.js** | Games page: renderGames, game identity modal |
