...
module.exports = {
  ...
  run() {
    ...
    checks.push(
      check("Mark completed Save (scheduled weekly): calendar fill + Trends hour/DOW", () => {
        const state = sim.createFixture({ today: "2026-08-03" });
        const game = sim.getGame(state);
        const task = game.weeklies[0];
        task.countFromDateStarted = true; // past empty weeks exist as skipped in History
        const key = sim.taskKey(game, task);

        // Ensure the Jul 20 week was incomplete (skipped) before Mark completed.
        assert.equal(sim.isCompletedInCycleForDate(state, "weeklies", key, "2026-07-22"), false);

        const res = sim.commitScheduledSkipToComplete(state, "weeklies", key, {
          finishDateStr: "2026-07-22",
          hour: 16,
          minute: 45,
        });
        assert.equal(res.ok, true, "Save ok");
        assert.ok(
          attendanceDayMarks(state, "2026-07-22").weeklies.includes(key),
          "calendar finish day marked"
        );
        assert.ok(
          attendanceDayMarks(state, "2026-07-26").weeklies.includes(key),
          "calendar fill-remaining through cycle"
        );
        assert.ok(
          !attendanceDayMarks(state, "2026-07-20").weeklies.includes(key),
          "days before finish stay unmarked"
        );
        // Neighbor earlier skipped week must not get painted.
        assert.equal(
          sim.isCompletedInCycleForDate(state, "weeklies", key, "2026-07-15"),
          false,
          "earlier skipped week stays incomplete"
        );

        const events = sim.getTrendEvents(state, "weeklies", key);
        assert.equal(events.length, 1, "one Trends event for the marked week");
        assert.equal(events[0].dateStr, "2026-07-22");
        assert.equal(events[0].hour, 16);
        const hours = sim.trendHourCounts(events);
        assert.equal(hours[16], 1, "Trends hour-of-day bucket");
        const dow = sim.trendDayOfWeekCounts(events);
        // 2026-07-22 = Wednesday
        assert.equal(dow[3], 1, "Trends DOW Wednesday");
        assert.equal(dow[0], 0, "Sunday fill does not appear in Trends DOW");
        assert.equal(sim.historyCompletionDayInCycle(state, "weeklies", key, "2026-07-22"), "2026-07-22");
      })
    );

    checks.push(
      check("Mark completed Save (scheduled endgame): calendar + Trends stack", () => {
        const state = sim.createFixture({ today: "2026-08-03" });
        const game = sim.getGame(state);
        const eg = game.endgame.find((t) => t.id === "endgame_a");
        eg.countFromDateStarted = true;
        const key = sim.taskKey(game, eg);

        assert.equal(sim.isCompletedInCycleForDate(state, "endgame", key, "2026-07-08"), false);

        const res = sim.commitScheduledSkipToComplete(state, "endgame", key, {
          finishDateStr: "2026-07-08",
          hour: 19,
          minute: 10,
          currencyValue: 800,
        });
        ...
        assert.ok(
          attendanceDayMarks(state, "2026-07-08").endgame.includes(key),
          "endgame finish day on calendar"
        );
        // 2-week cycle: fill after finish still marked
        assert.ok(
          attendanceDayMarks(state, "2026-07-12").endgame.includes(key),
          "endgame fill day on calendar"
        );

        const events = sim.getTrendEvents(state, "endgame", key);
        const hit = events.find((e) => e.dateStr === "2026-07-08");
        assert.ok(hit, "Trends event on finish day");
        assert.equal(hit.hour, 19);
        const stamp = (state.completionTimestamps || []).find(
          (t) => t.taskType === "endgame" && t.taskId === "endgame_a" && t.dateStr === "2026-07-08"
        );
        assert.ok(stamp && stamp.hour === 19);
        const stack = hourStackByType([stamp]);
        assert.equal(stack.endgame[19], 1, "Time Trends hour stack counts the finish");
        assert.equal(sim.historyCompletionDayInCycle(state, "endgame", key, "2026-07-08"), "2026-07-08");
      })
    );

    checks.push(
      check("Source: History Save Phase 2 applies scheduled skip→complete via applyTaskCompletion", () => {
        const fs = require("fs");
        const path = require("path");
        const modals = fs.readFileSync(path.join(__dirname, "..", "..", "src", "02-modals.js"), "utf8");
        assert.ok(modals.includes("Phase 2: skipped → completed"));
        assert.ok(modals.includes("applyTaskCompletion(type, key"));
        assert.ok(modals.includes("function openEarningsMarkCompleteModal"));
        assert.ok(modals.includes("function confirmEarningsMarkCompleteModal"));
      })
    );
